...
from mojave_review.data.window_fits import (
    bic_table, build_rerun_command, build_window_meta, list_window_fits,
    load_nwin_choices, nwin_choices_path, save_nwin_choices,
)
...
def _window_csv(path, ref_epoch, n_values, bic_min_at):
...
def _source_tree(tmp_path, labels_refs, complex_factor=3.0):
    ...
    for label, ref_epoch in labels_refs:
        ...
        _window_csv(fits / f"{SOURCE}.{label}.csv", ref_epoch,
                    n_values=range(1, 9), bic_min_at=4)
