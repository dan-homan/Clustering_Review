...
| Tab | What it shows |
|---|---|
| Robustness | "No changes suggested" checkbox at top + a list of *eligible* clusters (≥ 5 epochs with `use_in_fit=True`), one row each (`build_cluster_rows`, NOT a DataTable). Each row: clusterID, an **inline Robust / Non-robust radio preloaded to the model's current status**, and a comment textarea. There is no "—/no opinion" state — leaving the radio at its preloaded value *is* "no change"; only a pick that DIFFERS from current is recorded as an opinion and yields a derived `set_robust` edit (the row also highlights soft-red, via a clientside callback). Cluster 0 (the core) is always robust → its radio is disabled; its comment stays editable. See Robustness-tab plumbing under Epoch Notes. |
...
| Epoch Notes | One **real `dcc.Textarea`** per epoch (`build_epoch_rows`), NOT a DataTable cell — so the comment edits like a normal field (cursor / arrows / backspace / click-to-position / LTR). `epoch-feedback-table` is now a `dcc.Store` mirroring the old `.data` shape (`[{epoch, comment}]`) so every consumer is unchanged; `_sync_epoch_store` bridges the textareas (on `n_blur` → commit-on-blur, snappy typing) into that store. `_do_submit` reads the textareas' live `value` directly (the bridge is a server round-trip the submit clientside's microtask can't await). **The Robustness tab now uses the same pattern** (`build_cluster_rows` replaced the old DataTable dropdown, whose option popup rendered off-screen inside the scrollable panel): `cluster-feedback-table` is a `dcc.Store` mirroring the old `.data` shape (`[{clusterID, current_robust, recommended_robust, comment}]`) so every consumer is unchanged; `_sync_cluster_store` bridges the radios (immediate, on `value`) + comments (commit-on-blur, on `n_blur`) into it; load/resync/reset write `cluster-feedback-rows.children`; `_do_submit` reads the live radios + comments. The radio is preloaded to current, so `build_rec_from_ui_state` records an opinion only when a pick differs from `current_robust` (else `recommended_robust=None`), keeping `is_empty()` and aggregation votes correct. |
